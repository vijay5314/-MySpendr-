...
function AddCircleIcon({ size = 44 }) {
  return (
    <svg
      width={size}
      height={size}
...
/* ════ PIN LOCK ════ */
// FIX: accent is now received as a prop instead of relying on closure
function PinLock({ onUnlock, dark, accent }) {
  ...
  // Use a safe fallback if accent is somehow still undefined
...
/* ════ VOICE LOGGER ════ */
// FIX: accent is now received as a prop
          ...
          {principal > 0 && rate >= 0 && tenure > 0 && (
            ...
            >
              ...
              >
                ₹
                {Math.round(
                  calcEMI(Number(principal), Number(rate), Number(tenure))
                ).toLocaleString()}
              </p>
  ...
  return (
    <div
      style={{
        ...
        transition: deleting ? "opacity 0.2s" : "none",
      }}
    >
      <div
        style={{
      ...
      <div
        ...
        style={{
          transform: `translateX(${offset}px)`,
          transition: offset === 0 && !deleting ? "transform 0.3s" : "none",
          background: "inherit",
        }}
      >
        {children}
...
export default function App() {
  ...
  const monthlyTotal = useMemo(
    () =>
      expenses
        .filter(
          (e) => new Date(e.date + "T00:00:00").getMonth() === ist.getMonth()
        )
  ...
  const showGoldRateBanner = useMemo(() => {
    ...
    return !(
      updated.getMonth() === istNow.getMonth() &&
      updated.getFullYear() === istNow.getFullYear()
    );
  ...
  if (!unlocked) {
    // FIX: pass accent as prop to PinLock
    return (
      ...
      >
                ...
                <div style={{ position: "relative" }}>
                  <div
                    ...
                    style={{
                      ...
                      outline: amountShake ? "2px solid #ef4444" : "none",
                      cursor: "pointer",
                      display: "flex",
                      alignItems: "center",
                      justifyContent: "space-between",
                  ...
                  >
                    <span
                      style={{
                        color: amount ? textMain : dark ? "#4b5563" : "#9ca3af",
                        fontSize: 14,
                      }}
                    >
                      {amount
                        ...
                        {[50, 100, 150, 200, 300, 500].map((v) => (
                          <button
                            ...
                            style={{
                              ...
                              border: `1px solid ${border}`,
                              background:
                                amount === String(v) ? "#4f46e5" : "none",
                              color: amount === String(v) ? "#fff" : textMute,
                              fontSize: 12,
                              fontWeight: 600,
                              cursor: "pointer",
                            }}
                            ...
                            <button
                              ...
                              onClick={() => {
                                ...
                                if (k === "⌫") {
                                  ...
                                  const next = String(amount) + String(k);
                                  if (Number(next) <= MAX_AMOUNT)
                                    setAmount(next);
                                }
                              }}
                              style={{
          ...
          <>
            ...
            {showRForm ? (
              <div style={cardStyle}>
                ...
                >
                  Reminders will appear 3 days before and repeat daily until
                  paid or overdue.
                </p>
                <div style={{ display: "flex", gap: 8 }}>
                  <button
        ...
        {tab === "pot" && (
          <>
            ...
            {potSection === "networth" && (
              // FIX: pass accent as prop to SavingsGoals
              <SavingsGoals
                goals={goals}
        ...
        {tab === "scanvoice" && (
          <>
            ...
            {/* FIX: pass accent as prop to VoiceLogger */}
            <VoiceLogger
              categories={categories}
              ...
              accent={accent}
            />
            {/* FIX: pass accent as prop to ReceiptScanner */}
            <ReceiptScanner
              categories={categories}
        ...
        {tab === "emi" && (
          <>
            ...
            {/* FIX: pass accent as prop to EmiTab */}
            <EmiTab
              dark={dark}
              cardBg={cardBg}
              border={border}
        ...
        <>
          ...
          >
            ...
            <div style={{ padding: "8px 20px 16px" }}>
              ...
              >
                mySpendr v2.0 · your money, your streak
              </p>
            </div>
          </div>
        </>

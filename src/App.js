  ...
  return(
    <div style={{minHeight:"100vh",background:bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"'DM Sans',sans-serif",padding:24}}>
      ...
      <div style={{width:"100%",maxWidth:320,display:"flex",flexDirection:"column",alignItems:"center",gap:8}}>
        ...
        <div style={{display:"flex",gap:14,marginBottom:32,animation:shake?"shake 0.4s ease":"none"}}>
          ...
          {[0,1,2,3].map(i=>(
            <div key={i} style={{
              ...
              background:digits.length>i?safeAccent:(dark?"#1f2937":"#e5e7eb"),
              border:`2px solid ${digits.length>i?safeAccent:(dark?"#374151":"#d1d5db")}`,
        ...
        {mode==="enter"&&bioAvail&&(
          <button onClick={tryVerify} disabled={bioLoading} style={{marginTop:20,display:"flex",alignItems:"center",gap:8,background:"none",border:`1px solid ${dark?"#374151":"#e5e7eb"}`,borderRadius:12,padding:"10px 20px",cursor:"pointer",color:dark?"#9ca3af":"#6b7280",fontSize:13,fontWeight:500,opacity:bioLoading?0.6:1}}>
...
function CategoryBubbles({categories,catTotals,getCatStyle,getCatAccent,onSelect,dark,cardBg,border,textMute,subbg}){
  ...
  return(
    <div style={{marginBottom:12}}>
      <button onClick={()=>setOpen(o=>!o)} style={{width:"100%",display:"flex",justifyContent:"space-between",alignItems:"center",background:cardBg,border:`1px solid ${border}`,borderRadius:open?"16px 16px 0 0":16,padding:"10px 14px",cursor:"pointer"}}>
        ...
        <svg width="16"height="16"viewBox="0 0 24 24"fill="none"stroke={textMute}strokeWidth="2.5"strokeLinecap="round"strokeLinejoin="round"style={{transform:open?"rotate(180deg)":"rotate(0deg)",transition:"transform 0.2s"}}><polyline points="6 9 12 15 18 9"/></svg>
      </button>
      {open&&(
        <div style={{background:cardBg,border:`1px solid ${border}`,borderTop:"none",borderRadius:"0 0 16px 16px",padding:"12px 14px 14px"}}>
          {totalSpent>0&&(
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14,paddingBottom:14,borderBottom:`1px solid ${dark?"#1f2937":"#f3f4f6"}`}}>
              <svg width="180"height="180"viewBox="0 0 180 180"style={{flexShrink:0,overflow:"visible"}}>
                ...
                {segments.map(seg=>(
                  <circle key={seg.name}cx={CX}cy={CY}r={R}fill="none"stroke={seg.accent}strokeWidth={hovered===seg.name?SW+5:SW}strokeDasharray={`${seg.dash} ${circ}`}strokeDashoffset={seg.offset}strokeLinecap="butt"style={{transform:"rotate(-90deg)",transformOrigin:`${CX}px ${CY}px`,transition:"stroke-dasharray 0.6s ease,stroke-width 0.15s",cursor:"pointer"}}onMouseEnter={()=>setHovered(seg.name)}onMouseLeave={()=>setHovered(null)}onClick={()=>{onSelect(seg.name);setOpen(false);}}/>
                ))}
              ...
              <div style={{flex:1,display:"flex",flexDirection:"column",gap:6}}>
                {segments.map(seg=>(
                  <button key={seg.name} onMouseEnter={()=>setHovered(seg.name)} onMouseLeave={()=>setHovered(null)} onClick={()=>{onSelect(seg.name);setOpen(false);}} style={{display:"flex",alignItems:"center",gap:6,background:hovered===seg.name?(dark?"rgba(255,255,255,0.05)":"rgba(0,0,0,0.03)"):"transparent",border:"none",borderRadius:8,padding:"3px 6px",cursor:"pointer",width:"100%",textAlign:"left"}}>
          ...
          {sorted.map(cat=>{
            ...
            return(
              <button key={cat.name} onClick={()=>{onSelect(cat.name);setOpen(false);}} style={{width:"100%",display:"flex",alignItems:"center",gap:10,padding:"8px 0",background:"none",border:"none",cursor:"pointer",borderBottom:`1px solid ${dark?"#1f2937":"#f3f4f6"}`}}>
...
function VoiceLogger({categories,onAdd,dark,cardBg,border,textMute,textMain,inputBg,inputBorder,accent}){
  ...
  return(
    <div style={{background:cardBg,border:`1px solid ${border}`,borderRadius:16,padding:16,marginBottom:12}}>
      ...
      {supported&&(
        <>
          <button onClick={listening?stop:start} style={{width:"100%",padding:"14px",borderRadius:14,border:"none",cursor:"pointer",background:listening?"#ef4444":safeAccent,color:"#fff",fontSize:15,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center",gap:8,boxShadow:listening?"0 0 0 4px rgba(239,68,68,0.25)":"none",transition:"all 0.2s"}}>
...
function ReceiptScanner({categories,onAdd,dark,cardBg,border,textMute,textMain,inputBg,inputBorder,accent}){
  ...
  return(
    <div style={{background:cardBg,border:`1px solid ${border}`,borderRadius:16,padding:16,marginBottom:12}}>
      ...
      {!preview&&(
        <button onClick={()=>fileRef.current?.click()} style={{width:"100%",padding:"20px",borderRadius:14,border:`2px dashed ${border}`,background:"none",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:8,color:textMute}}>
          <CameraIcon size={32}/>
...
/* ════ EMI TRACKER TAB ════ */
function EmiTab({dark,cardBg,border,textMute,textMain,subbg,inputBg,inputBorder,categories,setExpenses,setPot,showToast,today,logDay,accent}){
  const safeAccent = accent || "#4f46e5";
  ...
  function saveEmi(){
    const p=Number(principal),r=Number(rate),t=Math.min(Number(tenure),600);
        ...
        :<div style={{background:cardBg,border:`1px solid ${border}`,borderRadius:16,padding:16,marginBottom:12}}>
          ...
          {principal>0&&rate>=0&&tenure>0&&(
            <div style={{background:dark?"#1f2937":"#f8fafc",borderRadius:10,padding:"10px 12px",marginBottom:8}}>
              ...
              <p style={{margin:"2px 0 0",fontSize:22,fontWeight:800,color:safeAccent,fontFamily:"monospace"}}>₹{Math.round(calcEMI(Number(principal),Number(rate),Math.min(Number(tenure),600))).toLocaleString()}</p>
              <p style={{margin:"2px 0 0",fontSize:11,color:textMute}}>Total payable: ₹{Math.round(calcEMI(Number(principal),Number(rate),Math.min(Number(tenure),600))*Math.min(Number(tenure),600)).toLocaleString()}</p>
            </div>
          )}
...
function SavingsGoals({goals,savings,dark,cardBg,border,textMute,textMain,subbg,inputBg,inputBorder,today,
  ...
  saveGoal,deleteGoal,editGoal,accent}){
  const safeAccent = accent || "#4f46e5";
  const inputStyle={background:inputBg,border:`1px solid ${inputBorder}`,color:textMain,borderRadius:12,padding:"8px 12px",fontSize:14,outline:"none",width:"100%",boxSizing:"border-box"};
  const btnPrimary={background:safeAccent,color:"#fff",border:"none",borderRadius:12,padding:"10px 16px",fontSize:14,fontWeight:600,cursor:"pointer"};
  const btnSecondary={background:dark?"#374151":"#f3f4f6",color:dark?"#d1d5db":"#374151",border:"none",borderRadius:12,padding:"8px 12px",fontSize:13,fontWeight:500,cursor:"pointer"};
        ...
        <div style={{background:cardBg,border:`1px solid ${border}`,borderRadius:16,padding:14,marginBottom:12}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
            <div>
              ...
              <input type="number" inputMode="decimal" value={goalTarget} onChange={e=>setGoalTarget(e.target.value)} placeholder="50000" style={inputStyle}/>
            </div>
            <div>
...
function SwipeableRow({onDelete,children,border,dark,cardBg}){
  ...
  function handleTouchEnd(){
    if(offset<-THRESHOLD){
      haptic([10,40,10]);
      setConfirmed(true);setDeleting(true);
  ...
  return(
    <div style={{position:"relative",overflow:"hidden",opacity:deleting?0:1,transition:deleting?"opacity 0.25s":"none"}}>
      ...
      <div
        ...
        onTouchMove={handleTouchMove}
        onTouchEnd={handleTouchEnd}
        style={{transform:`translateX(${offset}px)`,transition:offset===0&&!confirmed?"transform 0.3s cubic-bezier(0.25,0.46,0.45,0.94)":"none",background:cardBg||"inherit",position:"relative",zIndex:1}}
      >
        {children}
      </div>
    </div>
...
function ExpenseDateList({grouped,dailyTotal,today,dark,cardBg,border,subbg,textMute,getCatStyle,editExpense,deleteExpense,setDrillCat,formatDate}){
  ...
  useEffect(()=>{
    setOpenDates(prev=>prev[today]===true?prev:{...prev,[today]:true});
  },[today]);

  function toggle(dk){if(dk===today)return;setOpenDates(prev=>({...prev,[dk]:!prev[dk]}));}
          ...
          <div key={dk} style={{borderBottom:!isLast||isOpen?`1px solid ${border}`:"none"}}>
            ...
            >
              ...
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                ...
                {!isToday&&<svg width="14"height="14"viewBox="0 0 24 24"fill="none"stroke={textMute}strokeWidth="2.5"strokeLinecap="round"strokeLinejoin="round"style={{transform:isOpen?"rotate(180deg)":"rotate(0deg)",transition:"transform 0.2s",flexShrink:0}}><polyline points="6 9 12 15 18 9"/></svg>}
              </div>
            </div>
...
export default function App(){
  ...
  const[notifEnabled,setNotifEnabled]=useState(()=>{try{return localStorage.getItem(NOTIF_KEY)==='true';}catch{return false;}});

  const[today,setToday]=useState(()=>getTodayIST());
  useEffect(()=>{
    function scheduleNextDay(){
      const ms=msUntilMidnightIST();
      const t=setTimeout(()=>{setToday(getTodayIST());scheduleNextDay();},ms+500);
      return t;
    }
  ...
  function deductPot(source,amt,p){const f=source==="cash"?"usableCash":"usableBank";const current=Number(p[f])||0;return{...p,[f]:Math.max(0,current-amt)};}
  ...
  function validateAmount(val){const num=Number(val);return Number.isFinite(num)&&num>0&&num<=MAX_AMOUNT;}

  function saveExpense(){
    if(!validateAmount(amount)){setAmountShake(true);setTimeout(()=>setAmountShake(false),500);return;}
    const num=Number(amount);
    if(editingId){
      const old=expenses.find(e=>e.id===editingId);
      setPot(p=>{let updated=refundPot(old.paySource||"bank",old.amount,p);updated=deductPot(paySource,num,updated);return updated;});
  ...
  function switchTab(t,section){setTab(t);if(section)setPotSection(section);setTimeout(()=>{if(tabRef.current)tabRef.current.scrollIntoView({behavior:"smooth",block:"start"});},50);}
  ...
  const reminders=useMemo(()=>recurring.filter(r=>{
    ...
    return!paidTM;
  }).map(r=>({id:r.id,name:r.name,amount:r.amount,daysUntil:daysFromToday(r.nextDue),dueDateStr:formatDate(r.nextDue),category:r.category})).sort((a,b)=>a.daysUntil-b.daysUntil),[recurring,dismissedMap,today,ist]);
  ...
  useEffect(()=>{
    ...
    setNotifLog(prev=>{
      ...
      reminders.forEach(r=>{
        ...
        if(!prev.some(n=>n.key===key)){next.unshift({key,id:r.id,name:r.name,amount:r.amount,daysUntil:r.daysUntil,dueDateStr:r.dueDateStr,ts:Date.now()});changed=true;}
      });
  ...
  const catTotals=useMemo(()=>{const t={};expenses.forEach(e=>{t[e.category]=(t[e.category]||0)+e.amount;});return t;},[expenses]);
  const {grouped,dailyTotal}=useMemo(()=>{
    const g={},d={};
    [...expenses].sort((a,b)=>new Date(b.date)-new Date(a.date)).forEach(e=>{if(!g[e.date])g[e.date]=[];g[e.date].push(e);d[e.date]=(d[e.date]||0)+e.amount;});
  ...
  const monthlyTotal=useMemo(()=>expenses.filter(e=>{const ed=new Date(e.date+"T00:00:00");return ed.getMonth()===ist.getMonth()&&ed.getFullYear()===ist.getFullYear();}).reduce((s,e)=>s+e.amount,0),[expenses,ist]);
  ...
  const monthlyIncome=useMemo(()=>(pot.incomes||[]).reduce((s,i)=>{if(!i.active)return s;if(i.frequency==="Monthly")return s+i.amount;if(i.frequency==="Weekly")return s+i.amount*4;if(i.frequency==="Yearly")return s+Math.round(i.amount/12);return s;},0),[pot.incomes]);
  ...
  const extrasThisMonth=useMemo(()=>(pot.extras||[]).filter(e=>{const d=new Date(e.date+"T00:00:00");return d.getMonth()===ist.getMonth()&&d.getFullYear()===ist.getFullYear();}).reduce((s,e)=>s+e.amount,0),[pot.extras,ist]);
  const totalIn=monthlyIncome+extrasThisMonth;
  const totalOut=monthlyTotal;
  const daysElapsed=useMemo(()=>{const istNow=new Date(new Date().toLocaleString("en-US",{timeZone:"Asia/Kolkata"}));return Math.max(1,istNow.getDate());},[today]);
  const dailyAvg=daysElapsed>0?Math.round(monthlyTotal/daysElapsed):0;
  const trendData=useMemo(()=>{
    const months=[];const istNow=new Date(new Date().toLocaleString("en-US",{timeZone:"Asia/Kolkata"}));
    for(let i=5;i>=0;i--){
    ...
    <div style={{minHeight:"100vh",background:bg,color:textMain,fontFamily:"'DM Sans',sans-serif",transition:"background 0.3s"}}>
      ...
      <div style={{maxWidth:640,margin:"0 auto",padding:"24px 16px"}} key={tab} className="tabContent">
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
          ...
          <div style={{display:"flex",gap:6,alignItems:"center"}}>
            <button onClick={()=>{haptic(8);setShowNotifPanel(true);}} title="Notifications" style={{...btnSecondary,padding:"8px 10px",display:"flex",alignItems:"center",position:"relative"}}>
              <BellIcon/>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
              ...
              <div style={{background:cardBg,border:`1px solid ${border}`,borderRadius:16,padding:"14px 16px"}}>
                ...
                <p style={{margin:0,fontSize:22,fontWeight:800,fontFamily:"'DM Mono',monospace",color:(budget>0?remaining:totalIn-totalOut)<0?"#ef4444":"#16a34a",letterSpacing:"-0.5px"}}>
                  {budget>0?(remaining>=0?"+":"")+"₹"+Math.abs(remaining).toLocaleString():((totalIn-totalOut)>=0?"+":"")+"₹"+Math.abs(totalIn-totalOut).toLocaleString()}
                </p>
                <p style={{margin:"3px 0 0",fontSize:11,color:textMute}}>{budget>0?(remaining>=0?"available":"over budget"):"income − expenses"}</p>
              </div>
              <div onClick={()=>{if(topCategory){haptic(8);setDrillCat(topCategory);}}} style={{background:cardBg,border:`1px solid ${topCategory?(dark?"#374151":"#e0e7ff"):border}`,borderRadius:16,padding:"14px 16px",cursor:topCategory?"pointer":"default",transition:"border-color 0.15s"}}>
                <p style={{margin:0,fontSize:11,color:textMute,fontWeight:500,marginBottom:4}}>Top category</p>
                {topCategory
                  ?<><p style={{margin:0,fontSize:16,fontWeight:700,color:textMain,display:"flex",alignItems:"center",gap:4}}><span style={{...getCatStyle(topCategory),padding:"2px 8px",borderRadius:99,fontSize:12}}>{topCategory}</span></p><p style={{margin:"5px 0 0",fontSize:11,color:textMute}}>₹{(catTotals[topCategory]||0).toLocaleString()} · tap to view →</p></>
        ...
        {tab==="expenses"&&(
          <>
            <CategoryBubbles categories={categories} catTotals={catTotals} getCatStyle={getCatStyle} getCatAccent={getCatAccent} onSelect={(name)=>setDrillCat(name)} dark={dark} cardBg={cardBg} border={border} textMute={textMute} subbg={subbg}/>
            {expenses.length===0
              ...
              :<ExpenseDateList grouped={grouped} dailyTotal={dailyTotal} today={today} dark={dark} cardBg={cardBg} border={border} subbg={subbg} textMute={textMute} getCatStyle={getCatStyle} editExpense={(item)=>{editExpense(item);setTab("scanvoice");}} deleteExpense={deleteExpense} setDrillCat={setDrillCat} formatDate={formatDate}/>
            }
          </>
        )}
            ...
            {potSection==="usable"&&(
              <>
                <div style={{...cardStyle,background:dark?"linear-gradient(135deg,#111827,#1c1410)":"linear-gradient(135deg,#fffbeb,#fef3c7)",border:dark?"1px solid #292117":"1px solid #fde68a",display:"flex",flexDirection:"column",alignItems:"center",padding:"24px 16px 16px",gap:10}}>
                  <UsablePot fillPercent={usableFillPct} amount={usableTotal} size="lg"/>
                ...
                <div style={cardStyle}>
                  ...
                  {cashMode&&(
                    <div style={{display:"flex",gap:6,alignItems:"center"}}>
                      <input type="number" inputMode="decimal" value={cashAdj} onChange={e=>setCashAdj(e.target.value)} placeholder={`₹ to ${cashMode}`} style={{...inputStyle,flex:1}} autoFocus onKeyDown={e=>{if(e.key==="Enter"){quickAdjust("usableCash",cashMode,cashAdj);setCashAdj("");setCashMode(null);}}}/>
                ...
                <div style={cardStyle}>
                  ...
                  {bankMode&&(
                    <div style={{display:"flex",gap:6,alignItems:"center"}}>
                      <input type="number" inputMode="decimal" value={bankAdj} onChange={e=>setBankAdj(e.target.value)} placeholder={`₹ to ${bankMode}`} style={{...inputStyle,flex:1}} autoFocus onKeyDown={e=>{if(e.key==="Enter"){quickAdjust("usableBank",bankMode,bankAdj);setBankAdj("");setBankMode(null);}}}/>
              ...
              <>
                ...
                <div style={cardStyle}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
                    ...
                    <div style={{flex:1}}><span style={{fontSize:13}}>Gold</span>{pot.goldGrams>0&&pot.goldRate>0&&<span style={{fontSize:11,color:textMute,marginLeft:6}}>{pot.goldGrams}g × ₹{(Number(pot.goldRate)||0).toLocaleString()}/g</span>}</div>
                  ...
                  <div style={{paddingTop:10,borderTop:`1px solid ${border}`}}>
                    ...
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:6}}>
                      <div>
                        ...
                        <input type="number" inputMode="decimal" value={pot.goldGrams||""} onChange={e=>setPot(p=>({...p,goldGrams:Number(e.target.value)||0}))} placeholder="e.g. 24.5" style={{...inputStyle,textAlign:"right",fontWeight:700,color:"#d97706"}}/>
                      </div>
                      <div>
                        <p style={{margin:"0 0 4px",fontSize:11,color:textMute}}>Rate (₹/gram 24K)</p>
                        <input type="number" inputMode="decimal" value={pot.goldRate||""} onChange={e=>setPot(p=>({...p,goldRate:Number(e.target.value)||0,goldRateUpdatedOn:today}))} placeholder="e.g. 9200" style={{...inputStyle,textAlign:"right",fontWeight:700,color:"#d97706"}}/>
                      </div>
                    </div>
                ...
                <SavingsGoals goals={goals} savings={Number(pot.savings)||0} dark={dark} cardBg={cardBg} border={border} textMute={textMute} textMain={textMain} subbg={subbg} inputBg={inputBg} inputBorder={inputBorder} today={today} showGoalForm={showGoalForm} setShowGoalForm={setShowGoalForm} goalName={goalName} setGoalName={setGoalName} goalTarget={goalTarget} setGoalTarget={setGoalTarget} goalDeadline={goalDeadline} setGoalDeadline={setGoalDeadline} goalEditId={goalEditId} setGoalEditId={setGoalEditId} saveGoal={saveGoal} deleteGoal={deleteGoal} editGoal={editGoal} accent={accent}/>
              </>
            )}
          ...
          <>
            ...
            <div style={cardStyle}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
                <div>
                  ...
                  <input
                    ...
                    onChange={e=>{const v=e.target.value;if(v===""||Number(v)<=MAX_AMOUNT)setAmount(v);}}
              ...
              <div style={{marginBottom:10}}>
                ...
                <input value={note} onChange={e=>setNote(e.target.value)} placeholder="What was this for?" style={inputStyle} onKeyDown={e=>{if(e.key==="Enter")saveExpense();}}/>
              </div>
              <div style={{marginBottom:10}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
              ...
              <span style={{fontSize:11,fontWeight:600,color:textMute,textTransform:"uppercase",letterSpacing:"0.08em"}}>or use</span>
              <div style={{flex:1,height:1,background:border}}/>
            </div>
        ...
        {tab==="emi"&&(
          <>
            ...
            <EmiTab dark={dark} cardBg={cardBg} border={border} textMute={textMute} textMain={textMain} subbg={subbg} inputBg={inputBg} inputBorder={inputBorder} categories={categories} setExpenses={setExpenses} setPot={setPot} showToast={showToast} today={today} logDay={logDay} accent={accent}/>
          </>
        )}
          ...
          <div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:201,background:cardBg,borderRadius:"20px 20px 0 0",padding:"0 0 env(safe-area-inset-bottom,20px)",boxShadow:"0 -8px 32px rgba(0,0,0,0.18)"}}>
            ...
            <div style={{padding:"8px 20px 16px"}}>
              ...
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 0",borderBottom:`1px solid ${border}`}}>
                <div style={{display:"flex",alignItems:"center",gap:10}}>
                  {dark?<MoonIcon/>:<SunIcon/>}<span style={{fontSize:14,color:textMain}}>Dark mode</span>
                </div>
              ...
              <div style={{padding:"12px 0",borderBottom:`1px solid ${border}`}}>
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:editingName?8:0}}>
                  ...
                  <button onClick={()=>{setEditingName(e=>!e);setNameInput(userName);}} style={{background:dark?"#374151":"#f3f4f6",color:dark?"#d1d5db":"#374151",border:"none",borderRadius:10,padding:"5px 12px",fontSize:12,fontWeight:500,cursor:"pointer"}}>{editingName?"Cancel":"Edit"}</button>
                </div>
                {editingName&&(
                  <div style={{display:"flex",gap:6}}>
              ...
              <div style={{padding:"12px 0",borderBottom:`1px solid ${border}`}}>
                ...
                <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                  {ACCENTS.map(a=>(
                    <button key={a.id} onClick={()=>{haptic(6);setAccentId(a.id);}} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4,background:"none",border:"none",cursor:"pointer",padding:4}}>
              ...
              <p style={{margin:"16px 0 0",fontSize:11,color:textMute,textAlign:"center"}}>mySpendr v2.2 · your money, your streak</p>
            </div>
          </div>
        </>
      )}
      ...
      <div style={{position:"fixed",bottom:0,left:0,right:0,background:dark?"rgba(3,7,18,0.92)":"rgba(255,255,255,0.92)",backdropFilter:"blur(16px)",WebkitBackdropFilter:"blur(16px)",borderTop:`1px solid ${border}`,display:"flex",alignItems:"stretch",zIndex:100,paddingBottom:"env(safe-area-inset-bottom,0px)"}}>
        {[
          ...
          return(
            <button key={id} onClick={()=>{haptic(6);setTab(id);}} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:isScan?0:3,padding:isScan?"0 0 4px":"10px 0 8px",background:"none",border:"none",cursor:"pointer",color:active&&!isScan?(dark?"#818cf8":"#4f46e5"):textMute,transition:"color 0.15s",minWidth:0,position:"relative"}}>
              {icon}

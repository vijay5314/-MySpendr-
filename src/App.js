...
// FIX: Compute midnight-until-next-day ms for precise day-change detection
...
function PinLock({onUnlock,dark,accent}){
  ...
  // FIX: run biometric verify only once on mount when conditions are met
...
function buildAmortization(principal, annualRate, tenureMonths){
  // FIX: cap tenure to prevent UI hang on unreasonable inputs
...
function EmiTab({dark,cardBg,border,textMute,textMain,subbg,inputBg,inputBorder,categories,setExpenses,setPot,showToast,today,logDay,accent}){
  ...
  function saveEmi(){
    const p=Number(principal),r=Number(rate),t=Math.min(Number(tenure),600); // FIX: cap tenure
...
/* ════ SWIPEABLE ROW — FIX: clean non-overlapping reveal ════ */
function SwipeableRow({onDelete,children,border,dark,cardBg}){
  ...
  const THRESHOLD=80;
  const REVEAL=72; // how far to reveal the delete zone

  function handleTouchStart(e){
    startX.current=e.touches[0].clientX;
    // reset any open state when starting a new swipe
  }
  function handleTouchMove(e){
    if(startX.current===null)return;
    const dx=e.touches[0].clientX-startX.current;
    if(dx<0){
      // left swipe only, clamp to reveal width
  ...
  return(
    <div style={{
      ...
      {/* Delete zone — revealed underneath, not overlapping */}
      <div style={{
        position:"absolute",
        ...
        <TrashIcon/>
        <span style={{fontSize:9,fontWeight:700,color:"#fff",letterSpacing:"0.05em"}}>DELETE</span>
      </div>
      {/* Content slides left to reveal the delete zone */}
      <div
        onTouchStart={handleTouchStart}
        onTouchMove={handleTouchMove}
        onTouchEnd={handleTouchEnd}
        style={{
          ...
          transition:offset===0&&!confirmed?"transform 0.3s cubic-bezier(0.25,0.46,0.45,0.94)":"none",
...
function ExpenseDateList({grouped,dailyTotal,today,dark,cardBg,border,subbg,textMute,getCatStyle,editExpense,deleteExpense,setDrillCat,formatDate}){
  ...
  // FIX: stable key string avoids join on every render; split effect into two
  useEffect(()=>{
    setOpenDates(prev=>{
      const next={...prev};
...
export default function App(){
  ...
  // FIX: Replace 60s interval tick with precise midnight re-render
  ...
  // FIX: ist is computed from today string, not from a tick-driven Date()
  const ist=useMemo(()=>{
  ...
  useEffect(()=>{
    function onVis(){
      if(document.hidden){
        ...
        // FIX: also re-check date when app comes back to foreground
        setToday(getTodayIST());
      }
    }
  ...
  // FIX: stable callback ref for notification effect
  ...
  // FIX: use stable ref for reminders to avoid re-registering notif log on every render
  ...
  const monthlyTotal=useMemo(()=>expenses.filter(e=>{
    const ed=new Date(e.date+"T00:00:00");
    return ed.getMonth()===ist.getMonth()&&ed.getFullYear()===ist.getFullYear();
  }).reduce((s,e)=>s+e.amount,0),[expenses,ist]);
  ...
  if(!unlocked){
    return <PinLock onUnlock={()=>setUnlocked(true)} dark={dark} accent={accent}/>;
  }
        ...
        {tab==="expenses"&&(
          <>
            <div style={cardStyle}>
              ...
              {/* FIX: Native number input — clean, simple, lets the OS keyboard handle input */}
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:8}}>
                <div>
                  ...
                  <input
                    ...
                    style={{
                      ...
                      outline:amountShake?`2px solid #ef4444`:`1px solid ${amount&&Number(amount)>0?accent:inputBorder}`,
                      fontSize:18,
                      fontWeight:700,
                      fontFamily:"'DM Mono',monospace",
                      color:amount&&Number(amount)>0?(dark?"#f9fafb":"#111827"):textMute,
                    }}
                  />
                </div>
                <div>
          ...
          <>
            ...
            {potSection==="networth"&&(
              <>
                ...
                <SavingsGoals
                  goals={goals} savings={Number(pot.savings)||0} dark={dark} cardBg={cardBg} border={border}
                  textMute={textMute} textMain={textMain} subbg={subbg} inputBg={inputBg} inputBorder={inputBorder}
                  today={today} showGoalForm={showGoalForm} setShowGoalForm={setShowGoalForm}
                  goalName={goalName} setGoalName={setGoalName} goalTarget={goalTarget} setGoalTarget={setGoalTarget}
        ...
        <>
          ...
          <div style={{position:"fixed",bottom:0,left:0,right:0,zIndex:201,background:cardBg,borderRadius:"20px 20px 0 0",padding:"0 0 env(safe-area-inset-bottom,20px)",boxShadow:"0 -8px 32px rgba(0,0,0,0.18)"}}>
            ...
            <div style={{padding:"8px 20px 16px"}}>
              ...
              <p style={{margin:"16px 0 0",fontSize:11,color:textMute,textAlign:"center"}}>mySpendr v2.1 · your money, your streak</p>
            </div>
          </div>
        </>
      )}
